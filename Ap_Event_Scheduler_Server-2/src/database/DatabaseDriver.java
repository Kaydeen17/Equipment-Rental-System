    ...
    public synchronized boolean connectToDatabase() {
        try {
            if (getConn() == null || getConn().isClosed()) {
                ...
                try {
                    setConn(DriverManager.getConnection("jdbc:mysql://localhost:3307/", "root", "usbw"));
                ...
                setConn(DriverManager.getConnection("jdbc:mysql://localhost:3307/" + dbName, "root", "usbw"));
    ...
    public synchronized boolean executeSQLScript(String filePath) {
        try {
            if (getConn() != null && !getConn().isClosed()) {
                ...
                try (// Proceed with reading the SQL file
