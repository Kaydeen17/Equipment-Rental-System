...
CREATE TABLE IF NOT EXISTS Assets (
    ...
    user_id INT,
    availability INT GENERATED ALWAYS AS (
        (SELECT COUNT(*) 
         FROM asset_instances 
         WHERE asset_instances.asset_id = assets.asset_id 
           AND asset_instances.status = 'Available')
        - (SELECT COUNT(*) 
           FROM booking_asset_instances 
           WHERE booking_asset_instances.serial_number IN 
                (SELECT serial_number FROM asset_instances 
                 WHERE asset_instances.asset_id = assets.asset_id))
    ) STORED,
    FOREIGN KEY (user_id) REFERENCES Users(user_id) ON DELETE SET NULL
);
